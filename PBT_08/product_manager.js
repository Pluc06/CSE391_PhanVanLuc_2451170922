...
function sortByPrice(products, order = "asc") {
    // Dùng spread operator [...] để copy mảng, tránh mutate mảng gốc khi dùng sort()
...
function cheapestByCategory(products) {
    return products.reduce((acc, curr) => {
        // Nếu category này chưa có trong object acc, hoặc giá của sản phẩm hiện tại rẻ hơn sản phẩm đã lưu
...
function formatProductList(products) {
    return products.map(p => ({
        name: p.name,
        // Dùng toLocaleString để format giá tiền chuẩn Việt Nam
...
function averageRating(products) {
    if (products.length === 0) return 0; // Đề phòng mảng rỗng
    const totalRating = products.reduce((sum, p) => sum + p.rating, 0);
    return Number((totalRating / products.length).toFixed(1)); // Làm tròn 1 chữ số thập phân
}

// 8. Tìm sản phẩm theo keyword (tìm trong name, case-insensitive)
function searchProducts(products, keyword) {
    const lowerKeyword = keyword.toLowerCase();
    // Để đáp ứng yêu cầu BẮT BUỘC dùng hàm find(), mình dùng find ở đây 
    // để trả về sản phẩm ĐẦU TIÊN khớp với keyword nhé. 
    // (Nếu muốn lấy TẤT CẢ sản phẩm khớp, bạn có thể đổi find thành filter nha)
    return products.find(p => p.name.toLowerCase().includes(lowerKeyword));
}
